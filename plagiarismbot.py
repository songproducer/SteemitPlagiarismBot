...
while mainLoop < 1:
    ...
    while commentedYet < 1:
        while stopLookingForContent < 1:
            ...
            steem.connect(node="wss://this.piston.rocks")
            #steem.connect(node="wss://127.0.0.1:8091")
            ...
            if poster != stillTheSamePoster:
                ...
                if captureContent != None:
                    ...
                    try:
                        ...
                        #check if snippets are oftn coming up the same
                        for r in range(0,len(randomContentSnippetList)):
                            if randomContentSnippetList.count(r) > 1:
                                randomContentSnippetList.remove(r)
                                randomContentSnippetList.insert(r,(random.choice(captureContentInTheFinalList)))
                                randomContentSnippetIsDuplicate = randomContentSnippetIsDuplicate + 1
